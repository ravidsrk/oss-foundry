...
| 4 | Implement | Not a working gate. `applyAdvance` on `approved` stores a dry-run sandbox plan (`factory/engine.ts`) and bumps status to `implementing`. No playbook pack runs here. Wave 0 host / Wave 1+ E2B belong to a worker host that is not in this tree. The plan names omp OAuth/plan billing (not API keys); `harness-check` is the machine-specific pre-flight. CLI dry-run does not fake a green harvest |
