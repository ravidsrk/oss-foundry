...
| **G-17** | F-1-02, F-1-03, F-11-15, F-11-19, F-11-27 | 1, 11 | **CF-03** | S1 | FINISH (`human:H-03`) | No machine witness has ever been produced; `docs/evidence/logs/` does not exist, so the evidence page's `shasum` offer has no files behind it. **Blocked on `H-03` (a roster row), NOT on E2B** — corrected in the Phase 4 second look: `witness.ts:400-401` permits host witnessing at Wave 0, and `ravidsrk/orca-fleet` and `ravidsrk/frontguard` are both `wave: 0, sandbox: host` with real test commands, so a real machine witness is reachable today on repos the operator owns. E2B (`H-02`, non-gating) is required only to move a **Wave 1+** packet. Blocks launch gate §2 for CF-03. |
...
| **G-43** | S5-B §3 | 6 | all | S2 | FINISH | **closed** | PLAN-named evidence files were never written. Closed as tracker drift: files now exist. Gate §3's *carrying* proof remains the concurrent reader (0 partial / 186; S5-B already marked §3 MET). The kill test asserts absent-or-loadable over small N — it is not a 12-iteration soak and is not treated as one. |
...
| **G-46** | S5-B §2 | 1 | CF-02..05 | S2 | FINISH | **closed** | Closed the *null file* finding at `f15cec1`. Captured reachable refusals (merged/unknown/bad SHA/disclosure suite). These are **not** the frozen DEFINITION.md §1 failure paths that need a gated or draft-ready packet — those remain open under G-16/G-17/G-18. |
