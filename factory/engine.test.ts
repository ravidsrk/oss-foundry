...
import { blank, BASE, HEAD, OTHER, bindingFor, witnessed, live, reviewing, readyEvidence } from "./engine-test-helpers.ts";

test("G-39: engine tests are split across files, not one 5800-line module", () => {
  const here = fileURLToPath(new URL(".", import.meta.url));
  const core = readFileSync(join(here, "engine.test.ts"), "utf8").split("\n").length;
  const witness = readFileSync(join(here, "engine-witness.test.ts"), "utf8").split("\n").length;
  assert.ok(core < 4000, `engine.test.ts is ${core} lines — the split receded`);
  assert.ok(witness > 1500, `engine-witness.test.ts is ${witness} lines — expected the witness half`);
});
...
test("#121: a 403 after closed-unmerged is a block, not a decline", () => {
  const state = withOpenSubmittedWave1();
  const submitted = state.packets.find((p) => p.status === "submitted")!;
  const closed = applyPrSync(
    state,
    submitted.id,
    prMetaAt("2026-09-01T00:00:00.000Z", { state: "closed", merged: false }),
    { reviewTruncated: false, threadsAnswered: true, at: "2026-09-02T00:00:00.000Z" },
  );
  const rowAfterClose = closed.state.scorecard.find((r) => r.repoId === submitted.repoId);
  assert.equal(rowAfterClose?.closedUnmerged, 1);
  assert.equal(rowAfterClose?.maintainerTone === "banned", false);

  assert.equal(isBlockSignal("GitHub 403 creating the draft on ColeMurray/background-agents: forbidden"), true);
  assert.equal(isBlockSignal("GitHub 403 listing pulls: primary rate limit exhausted (core: 60 requests/hour, remaining 0, resets soon)"), false);
  ...
  const blocked = applyObservedBlock(
    closed.state,
    submitted.repoId,
    "GitHub 403 creating the draft on ColeMurray/background-agents: forbidden",
  );
  assert.equal(blocked.recorded, true, "a 403 after closed-unmerged must record a block");
  const row = blocked.state.scorecard.find((r) => r.repoId === submitted.repoId);
  assert.equal(row?.maintainerTone, "banned");
  assert.equal(row?.closedUnmerged, 0, "the decline must not stand as the record");
  assert.equal(health(row!), "stop");
  const packet = blocked.state.packets.find((p) => p.id === submitted.id);
  assert.equal(packet?.status, "parked");
});

test("#121: a 403 with no prior closed-unmerged is not a block", () => {
  const state = withOpenSubmittedWave1();
  const submitted = state.packets.find((p) => p.status === "submitted")!;
  const observed = applyObservedBlock(
    state,
    submitted.repoId,
    "GitHub 403 creating the draft on ColeMurray/background-agents: Resource not accessible",
  );
  assert.equal(observed.recorded, false);
  const row = observed.state.scorecard.find((r) => r.repoId === submitted.repoId);
  assert.notEqual(row?.maintainerTone, "banned");
});

test("#121: an ordinary close is still a closedUnmerged decline", () => {
  const state = withOpenSubmittedWave1();
  const submitted = state.packets.find((p) => p.status === "submitted")!;
  const closed = applyPrSync(
    state,
  ...
  const row = closed.state.scorecard.find((r) => r.repoId === submitted.repoId);
  assert.equal(row?.closedUnmerged, 1);
  assert.notEqual(row?.maintainerTone, "banned");
  assert.equal(closed.state.packets.find((p) => p.id === submitted.id)?.status, "followed-up");
});
