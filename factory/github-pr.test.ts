...
test("compareCommits accepts only a fast-forward ahead range", async () => {
  const ahead = await compareCommits("ravidsrk/orca-fleet", BASE, HEAD, async () =>
    jsonResponse(200, { status: "ahead", ahead_by: 2, behind_by: 0 }),
  );
  assert.equal(ahead.ok, true);
  if (ahead.ok) assert.equal(ahead.aheadBy, 2);

  const diverged = await compareCommits("ravidsrk/orca-fleet", BASE, HEAD, async () =>
    jsonResponse(200, { status: "diverged", ahead_by: 3, behind_by: 1 }),
  );
