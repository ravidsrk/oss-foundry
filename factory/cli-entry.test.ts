...
test("spawning cli.ts as the entry point still runs main()", () => {
  ...
  for (const verb of ["status", "events", "tick", "approve", "reject", "halt", "revert", "advance", "evidence", "witness-check", "harness-check", "attach-witness", "body", "attach-draft", "open-draft", "sync", "reconcile", "ledger", "evidence-page", "clear-halt"]) {
